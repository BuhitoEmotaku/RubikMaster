...
header("Content-Type: application/json"); // Indicar que la respuesta es JSON
                ...
                //header("Access-Control-Allow-Origin: *");
                // Permitir los métodos POST, GET y OPTIONS
                //header("Access-Control-Allow-Methods: POST, GET, OPTIONS");
                // Permitir los encabezados Content-Type y Authorization
                //header("Access-Control-Allow-Headers: Content-Type, Authorization");
                // Si no existe el POST 'urlsImgsCubes', simplemente imprimimos la URL de la imagen
...
$checkUrlsImgsCubes = isset($_POST['urlsImgsCubes']);
$data = json_decode(file_get_contents('php://input'), true);
$urlsImgsCubes = isset($data['urlsImgsCubes']);

$urlsImgsCubesDecode;
$urlsArray;
$longitude;
if($urlsImgsCubes){
    $urlsImgsCubesDecode = $data['urlsImgsCubes'];
    $urlsArray = explode(',', $urlsImgsCubesDecode);
    $longitude = count($urlsArray);
    
} 
else $longitude = 1;

// Iterar sobre el array usando un bucle for
for ($e = 0; $e < $longitude; $e++) {
    if ($urlsImgsCubes) {
        // Obtener los parámetros de la URL actual
        $parsedUrl = parse_url($urlsArray[$e]);
        parse_str($parsedUrl['query'], $params);

        // Establecer los valores de $_REQUEST según los parámetros de la URL actual
    ...
    if (
        array_key_exists('r', $_REQUEST) || array_key_exists('r', $DEFAULTS)
        || ($ENABLE_COOKIES && isset($_COOKIE['vc_r']) && $_COOKIE['vc_r'] != '')
    ) {
        $_r = array_key_exists('r', $_REQUEST) ? $_REQUEST['r'] : ($ENABLE_COOKIES && isset($_COOKIE['vc_r']) && $_COOKIE['vc_r'] != '' ?
    ...
    $dim = $DEFAULTS['pzl'];
    if (
        array_key_exists('pzl', $_REQUEST) && is_numeric($_REQUEST['pzl'])
    ...
    if (
        array_key_exists('sch', $_REQUEST) || array_key_exists('sch', $DEFAULTS)
        || ($ENABLE_COOKIES && isset($_COOKIE['vc_sch']) && $_COOKIE['vc_sch'] != '')
    ) {
        // Retrieve from cookie or 'sch' variable
        $sd = array_key_exists('sch', $_REQUEST) ? $_REQUEST['sch'] : ($ENABLE_COOKIES && isset($_COOKIE['vc_sch']) && $_COOKIE['vc_sch'] != '' ?
    ...
    if (array_key_exists('stage', $_REQUEST) || array_key_exists('stage', $DEFAULTS)) {
        ...
        if ($mask) {
            for ($i = 0; $i < $dim * $dim * 6; $i++) {
                $facelets[$i] = $mask[$i] == 0 ?
                    ($view == 'trans' ? ($using_cols ? 't' : $T) : ($using_cols ? 'l' : $N)) : $facelets[$i];
            }
        }
    }
    ...
    // Retrieve alg def
    if (
        array_key_exists('alg', $_REQUEST) || array_key_exists('case', $_REQUEST)
        || array_key_exists('alg', $DEFAULTS) || array_key_exists('case', $DEFAULTS)
    ) {
        require_once "cube_lib.php";
        if (array_key_exists('alg', $_REQUEST)) $alg = $_REQUEST['alg'];
        else $alg = $DEFAULTS['alg'];
        if (array_key_exists('case', $_REQUEST)) $case = $_REQUEST['case'];
        else $case = $DEFAULTS['case'];
    ...
    for ($fc = 0; $fc < 6; $fc++) {
        for ($i = 0; $i <= $dim; $i++) {
            for ($j = 0; $j <= $dim; $j++) {
                switch ($fc) {
                    case $U:
                        $p[$fc][$i][$j] = array($i,    0, $dim - $j);
                        break;
                    case $R:
                        $p[$fc][$i][$j] = array($dim,   $j,        $i);
                        break;
                    case $F:
                        $p[$fc][$i][$j] = array($i,   $j,         0);
                        break;
                    case $D:
                        $p[$fc][$i][$j] = array($i, $dim,        $j);
                        break;
                    case $L:
                        $p[$fc][$i][$j] = array(0,   $j, $dim - $i);
                        break;
                    case $B:
                        $p[$fc][$i][$j] = array($dim - $i,   $j,      $dim);
                        break;
                }
...
<svg version='1.1' xmlns='http://www.w3.org/2000/svg'
    ...
    // Draw background
    if ($bg) $cube .= "\t<rect fill='transparent' x='$ox' y='$oy' width='$vw' height='$vh'/>\n";
    ...
    if ($DEBUG) echo $cube;
        ...
        if ($urlsImgsCubes) {
            // Si existe el POST 'urlsImgsCubes', agregamos la imagen al array $imgsCubesParsed
            array_push($imgsCubesParsed , display_img($img, $fmt));
        } else {
            $result = array(
                'svg' => display_img($img, $fmt),
            );
        
            // Devolver la respuesta como JSON
            echo json_encode($result);
            break; // Terminar el bucle después de una sola iteración si no hay array
        }
...
if ($urlsImgsCubes) {
    ...
    $responseArray = $imgsCubesParsed;
    
    // Codificar el array asociativo como JSON
